  ...
  try {
    ...
    if (req.roleId === 1) {
      response = await Doctors.findAll({
        attributes: ["id", "uuid", "userId", "strNumber", "birthDate", "address", "specialization", "practicePlace", "note", "createdAt", "updatedAt"],
        include: [
          {
            model: Users,
...
export const createDoctor = async (req, res) => {
  const { userId, strNumber, birthDate, address, specialization, practicePlace, note } = req.body;
